...
def crawl() -> None:
    auth_response = requests.post(
        "https://yes-commission.api.pinxed.com/users/login",
        data={
            "username": os.getenv("YES_EDU_USERNAME"),
            "password": os.getenv("YES_EDU_PASSWORD"),
        },
    )
    access_token = auth_response.json()["token"]
    schools_response = requests.get(
        "https://yes-commission.api.pinxed.com/school/",
...
def create_or_update_school(school_data, state: str):
    ...
    school_record, school_created = School.objects.get_or_create(
        ...
        state=state,
        defaults={"sources": ["yes_edu"]},
    )

    if school_created:
        logger.info(
            "Create a new school %s in %s, %s", school_name, school_country, state
        )
    ...
    if "yes_edu" not in school_record.sources:
        logger.info(
            "Update school source with yes_edu for school: %s", school_record.name
        )
    ...
    for course_data in school_data["course"]:
        ...
        course_sector = resolve_course_sector_from_name(
            course_name, school_data["type"]
        )
        ...
        if course_record.commission != course_data["commission"]:
            logger.info(
                "Update course commision with %s for school: %s",
                course_data["commission"],
                school_record.name,
            )
...
def resolve_course_sector_from_name(name: str, school_type: str):
    ...
    if (
        ("master" in lower_name)
        or ("phd" in lower_name)
        or ("postgraduate" in lower_name)
    ):
